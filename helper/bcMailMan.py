...
def bcMailMan(caseRootDir : str, Mach : float, T : float = 300.0, I : float = 0.001, mixingLength : float = 1e-5) -> None:
    ...
    os.system(f'foamDictionary {fileDir} -entry Ux -set {U:.2f}')
    os.system(f'foamDictionary {fileDir}  -entry k -set {k:.3f}')
    os.system(f'foamDictionary {fileDir}  -entry omega -set {int(omega):d}')
    if T != 300 : os.system(f'foamDictionary {fileDir}  -entry T -set {T}')
